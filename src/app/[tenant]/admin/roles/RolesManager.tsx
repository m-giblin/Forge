...
type Props = { slug: string; initialRoles: (CustomRole & { memberCount: number })[] };

const EMPTY_PERMS: RbacPermissionSet = Object.fromEntries(RBAC_PERMISSIONS.map((p) => [p, false])) as RbacPermissionSet;
...
function PermissionsGrid({
  ...
  const toggle = (perm: (typeof RBAC_PERMISSIONS)[number]) => {
    onChange({ ...value, [perm]: !value[perm] });
  };
  return (
    <div className="space-y-4">
      {PERMISSION_GROUPS.map((group: PermissionGroup) => {
        const perms = RBAC_PERMISSIONS.filter((p) => PERMISSION_META[p].group === group);
        return (
          <div key={group}>
            <p className="mb-1.5 text-[10px] font-semibold uppercase tracking-widest text-neutral-400">{group}</p>
            <div className="space-y-1">
              {perms.map((perm) => (
                <label key={perm} className="flex cursor-pointer items-center justify-between rounded-lg px-3 py-2 hover:bg-neutral-50">
                  <span>
                    <span className="text-sm font-medium text-neutral-700">{PERMISSION_META[perm].label}</span>
                    <span className="ml-2 text-xs text-neutral-400">{PERMISSION_META[perm].description}</span>
                  </span>
                  <input
                    type="checkbox"
                    checked={value[perm] ?? false}
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <div className="rounded-xl border border-neutral-200 bg-neutral-50 p-3">
          <PermissionsGrid value={form.permissions} onChange={(p) => setForm((f) => ({ ...f, permissions: p }))} />
        </div>
      </div>
...
export default function RolesManager({ slug, initialRoles }: Props) {
    ...
    <div className="space-y-4">
            ...
            <div className="flex items-center justify-between px-4 py-3">
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs text-neutral-400">
                  ...
                  {" · "}
                  {RBAC_PERMISSIONS.filter((p) => role.permissions[p]).length} permission{RBAC_PERMISSIONS.filter((p) => role.permissions[p]).length !== 1 ? "s" : ""}
                </span>
              </div>
      ...
      {creating ? (
        <div className="rounded-xl border border-indigo-200 bg-white p-4">
          ...
          <RoleForm
            initial={{ name: "", description: "", color: "indigo", permissions: { ...EMPTY_PERMS } }}
            pending={pending}
