...
import { loadTimeReportData, loadSprintRollupAction, type TimeReportData, type TimeReportSprint, type SprintRollupRow } from "./timeReports";
...
type SubTab = "user" | "project" | "sprint";
...
function exportLogsCsv(data: TimeReportData, label: string) {
  downloadCsv([
  ...
  ], `time-report-${label}.csv`);
}

function exportUsersCsv(data: TimeReportData, label: string) {
  downloadCsv([
    ["Member", "Total Hours", "Billable Hours", "Non-Billable Hours", "Log Count"],
    ...data.members.map((m) => [
      m.name,
      (m.totalMinutes / 60).toFixed(2),
      (m.billableMinutes / 60).toFixed(2),
      ((m.totalMinutes - m.billableMinutes) / 60).toFixed(2),
      String(m.logCount),
    ]),
  ], `time-by-user-${label}.csv`);
}

function exportProjectsCsv(data: TimeReportData, label: string) {
  downloadCsv([
    ["Project", "Total Hours", "Billable Hours"],
    ...data.projects.map((p) => [
      `${p.projectKey} – ${p.projectName}`,
      (p.totalMinutes / 60).toFixed(2),
      (p.billableMinutes / 60).toFixed(2),
    ]),
  ], `time-by-project-${label}.csv`);
}

function exportSprintCsv(rows: SprintRollupRow[]) {
  downloadCsv([
    ["Sprint", "Project", "Start", "End", "Total Hours", "Top Contributor"],
    ...rows.map((r) => [
      r.sprintName,
      r.projectKey,
      r.startDate ?? "",
...
function HorizBar({ label, value, max, billable }: { label: string; value: number; max: number; billable: number }) {
...
export default function TimeReportsTab({ slug, initialSprints, activeSprint }: Props) {
  ...
  const [sprintRollup, setSprintRollup] = useState<SprintRollupRow[] | null>(null);
  ...
  useEffect(() => {
    if (subTab !== "sprint" || sprintRollup !== null) return;
    let cancelled = false;
    Promise.resolve().then(() => {
      if (!cancelled) setSprintLoading(true);
      return loadSprintRollupAction(slug);
    }).then((rows) => {
      if (!cancelled) { setSprintRollup(rows); setSprintLoading(false); }
    });
    return () => { cancelled = true; };
  }, [subTab, slug, sprintRollup]);

  function handleModeChange(m: Mode) {
    setMode(m);
    if (m !== "sprint") setSelectedSprintId(null);
    if (m !== "custom") load(m, m === "sprint" ? selectedSprintId : null, "", "");
  }
  ...
  const filteredLogs = filterMember ? (data?.logs ?? []).filter((l) => l.userId === filterMember) : (data?.logs ?? []);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="ml-auto flex items-center gap-2">
          {isPending && <span className="text-xs text-neutral-400 animate-pulse">Loading…</span>}
        </div>
      </div>
      ...
      {!data && !isPending && (
        <p className="text-sm text-neutral-400 py-8 text-center">No data yet.</p>
      )}

      {data && (
        <>
          {/* Daily trend */}
          {data.dailyTrend.length > 1 && !filterMember && subTab === "user" && (
                ...
                <div className="rounded-xl border border-neutral-200 bg-white p-5">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => exportUsersCsv(data, sprintLabel)}
                        className="inline-flex items-center gap-1 rounded-lg border border-neutral-200 bg-white px-2.5 py-1 text-xs font-medium text-neutral-600 hover:bg-neutral-50">
                        ↓ CSV
                      </button>
                    </div>
                  </div>
                  <div>
                    {data.members.map((m) => {
                      const memberLogs = data.logs.filter((l) => l.userId === m.userId);
                      const byProject = new Map<string, number>();
                      for (const l of memberLogs) {
                        byProject.set(l.projectKey, (byProject.get(l.projectKey) ?? 0) + l.minutes);
                      }
                      const isExpanded = expandedUser === m.userId;
                      return (
                        <div key={m.userId}>
                          <button className="w-full text-left hover:bg-neutral-50 rounded-lg px-1 transition"
                            onClick={() => setExpandedUser(isExpanded ? null : m.userId)}>
                            <HorizBar label={m.name} value={m.totalMinutes} max={data.members[0].totalMinutes} billable={m.billableMinutes} />
                          </button>
                          {isExpanded && (
              ...
              <div className="rounded-xl border border-neutral-200 bg-white overflow-hidden">
                <div className="flex items-center justify-between px-5 py-3 border-b border-neutral-100">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => exportLogsCsv(data, sprintLabel)}
                      className="inline-flex items-center gap-1 rounded-lg border border-neutral-200 bg-white px-2.5 py-1 text-xs font-medium text-neutral-600 hover:bg-neutral-50">
                      ↓ CSV
                    </button>
                  </div>
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    ...
                    {filteredLogs.length === 0 && (
                      <tr><td colSpan={7} className="px-4 py-8 text-center text-neutral-400">No entries for this period.</td></tr>
                    )}
                  </tbody>
                </table>
          ...
          {subTab === "project" && (
            <div className="rounded-xl border border-neutral-200 bg-white p-5">
              <div className="flex items-center justify-between mb-3">
                <p className="text-xs font-semibold uppercase tracking-wide text-neutral-500">By Project</p>
                <button onClick={() => exportProjectsCsv(data, sprintLabel)}
                  className="inline-flex items-center gap-1 rounded-lg border border-neutral-200 bg-white px-2.5 py-1 text-xs font-medium text-neutral-600 hover:bg-neutral-50">
                  ↓ CSV
                </button>
              </div>
              {data.projects.length === 0 ? (
                <p className="text-xs text-neutral-400 py-4 text-center">No data in this period.</p>
              ) : (
                data.projects.map((p) => (
                  <HorizBar key={p.projectKey} label={`${p.projectKey} · ${p.projectName}`} value={p.totalMinutes} max={data.projects[0].totalMinutes} billable={p.billableMinutes} />
                ))
              )}
            </div>
          )}
          ...
          {subTab === "sprint" && (
            <div className="rounded-xl border border-neutral-200 bg-white overflow-hidden">
              <div className="flex items-center justify-between px-5 py-3 border-b border-neutral-100">
                <p className="text-xs font-semibold uppercase tracking-wide text-neutral-500">By Sprint</p>
                {sprintRollup && sprintRollup.length > 0 && (
                  <button onClick={() => exportSprintCsv(sprintRollup)}
                    className="inline-flex items-center gap-1 rounded-lg border border-neutral-200 bg-white px-2.5 py-1 text-xs font-medium text-neutral-600 hover:bg-neutral-50">
                    ↓ CSV
                  </button>
                )}
              </div>
              {sprintLoading && <p className="px-5 py-8 text-xs text-neutral-400 text-center">Loading…</p>}
              {!sprintLoading && sprintRollup !== null && sprintRollup.length === 0 && (
                <p className="px-5 py-8 text-xs text-neutral-400 text-center">No sprints found.</p>
              )}
              {!sprintLoading && sprintRollup !== null && sprintRollup.length > 0 && (
                <table className="w-full text-xs">
                  <thead>
                    <tr className="bg-neutral-50 text-left">
                      {["Sprint", "Project", "Date Range", "Total Hours", "Top Contributor"].map((h) => (
                  ...
                  <tbody>
                    {sprintRollup.map((r) => (
                      <tr key={r.sprintId} className="border-t border-neutral-50 hover:bg-neutral-50">
                        ...
                        <td className="px-4 py-2.5 font-semibold text-neutral-800 tabular-nums">{fmtMins(r.totalMinutes)}</td>
                        <td className="px-4 py-2.5 text-neutral-600">{r.topContributor ?? "—"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
